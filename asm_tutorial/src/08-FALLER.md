...
A red ball falls from the top of the screen while we move the bucket left and right — our first two-sprite program. This chapter builds on `bucket.asm` from Chapter 7.

## Building on Chapter 7

Before making changes, save a copy of your current file:

```bash
cp src/bucket.asm src/faller.asm
```

Open `src/faller.asm`. We'll add a second sprite (a falling ball), set up a random number source, and animate the ball's descent. The bucket movement code stays the same — we're only adding to it.

### New Sprite Data

Add the ball sprite data between the bucket data and the code section. The ball goes at `$0880` — the next 64-byte slot after the bucket:
...
This is exactly why we left room between `$0840` and `$0900` in Chapter 7 — the ball slots into `$0880` without moving anything.

### New Variable

Add a variable for the ball's Y position:

```asm
ball_y     = $10                ; Ball Y position
```

We use `$10` instead of `$04` to keep related variables in groups — `$02`-`$03` for the bucket's position, `$10`+ for ball state. This will be helpful as we add more variables in later chapters.

### Ball Initialization

After the bucket setup, add the ball sprite initialization:
...
Then change the sprite enable to include both sprites:
...
Add this after the sprite enable to configure SID voice 3 as a random number source:
...
### Ball Animation in the Game Loop

Replace the `jmp update` (the "no input" path) with `jmp move_ball`, and add the ball animation section before the `update:` label:
...
Also change the `jmp move_ball` target in the left/right move sections.

### Delay Adjustment

Reduce the delay constant from `$08` to `$06` — the second sprite adds a bit more visual interest, and a faster loop makes the game feel more responsive:

```asm
delay:
    ldx #$06                    ; Outer loop (was $08)
```

## The Complete Code

Here's the full listing. Changes from Chapter 7 are marked with `; ← NEW`:
...
* = $0880                       ; Ball sprite (pointer = 34)       ← NEW

ball_data:                                                         ; ← NEW
    !fill 21, 0                 ; Rows 0-6: empty                  ← NEW
    ; Row 7: top of ball
    !byte %00000000,%00111100,%00000000                             ; ← NEW
    ; Row 8
    !byte %00000000,%01111110,%00000000                             ; ← NEW
    ; Rows 9-12: middle
    !byte %00000000,%11111111,%00000000                             ; ← NEW
    !byte %00000000,%11111111,%00000000                             ; ← NEW
    !byte %00000000,%11111111,%00000000                             ; ← NEW
    !byte %00000000,%11111111,%00000000                             ; ← NEW
    ; Row 13
...
ball_y     = $10                ; Ball Y position                  ← NEW
    ...
    ; --- Initialize ball sprite (sprite 1) ---                    ← NEW

    lda #34                     ; Ball data at $0880 (34 x 64)     ← NEW
    sta $07f9                   ; Sprite 1 pointer                 ← NEW

    lda #50                     ; Start near top of screen         ← NEW
    sta ball_y                                                     ; ← NEW
    sta $d003                   ; Sprite 1 Y                       ← NEW

    lda #172                    ; Start at center X                ← NEW
    sta $d002                   ; Sprite 1 X                       ← NEW

    lda #$02                    ; Red                              ← NEW
    sta $d028                   ; Sprite 1 color                   ← NEW

    ; --- Enable sprites 0 and 1 ---

    lda $d015                   ; Read sprite enable register
    ora #%00000011              ; Set bits 0 and 1                 ← CHANGED
    sta $d015

    ; --- Set up SID voice 3 for random numbers ---                ← NEW

    lda #$ff                    ; Maximum frequency                ← NEW
    sta $d40e                   ; Voice 3 frequency low            ← NEW
    sta $d40f                   ; Voice 3 frequency high           ← NEW
    lda #$80                    ; Noise waveform, gate off         ← NEW
    sta $d412                   ; Voice 3 control register         ← NEW
...
loop:
    ...
    jmp move_ball               ; No horizontal input              ← CHANGED
...
move_ball:                                                         ; ← NEW
    ; --- Animate ball: move down one pixel ---                    ← NEW

    inc ball_y                  ; Increment Y position             ← NEW
    lda ball_y                                                     ; ← NEW
    sta $d003                   ; Update sprite 1 Y register       ← NEW

    ; Check if ball reached bottom of screen                       ← NEW
    cmp #250                    ; Past visible area?               ← NEW
    bcc update                  ; No: skip reset                   ← NEW

    ; --- Reset ball to top with new random X ---                  ← NEW

    lda #50                     ; Back to top                      ← NEW
    sta ball_y                                                     ; ← NEW
    sta $d003                                                      ; ← NEW

    lda $d41b                   ; Read SID voice 3 random value    ← NEW
    sta $d002                   ; New X position for ball          ← NEW

    lda $d010                   ; Read X position MSB register     ← NEW
    and #%11111101              ; Clear bit 1 (sprite 1)           ← NEW
    sta $d010                   ; Keep ball in 0-255 X range       ← NEW
...
```
Row 7:  ....####....  %00000000 %00111100 %00000000
Row 8:  ...######...  %00000000 %01111110 %00000000
Row 9:  ..########..  %00000000 %11111111 %00000000
Row 10: ..########..  %00000000 %11111111 %00000000
Row 11: ..########..  %00000000 %11111111 %00000000
Row 12: ..########..  %00000000 %11111111 %00000000
Row 13: ...######...  %00000000 %01111110 %00000000
Row 14: ....####....  %00000000 %00111100 %00000000
```

The first and third bytes are always `%00000000` — the ball only uses the center 8 pixels. The rows widen and narrow to approximate a circle.
...
Your .prg file should be **444 bytes**.
