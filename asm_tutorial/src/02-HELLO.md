...
- `!byte $10, $08` - emit literal byte values
- `!text "2066"` - emit ASCII text bytes
...
| Bytes | Meaning |
|-------|---------|
| `$10, $08` | Pointer to next BASIC line ($0810). BASIC uses this to find the next line. |
| `$0a, $00` | Line number 10 in little-endian (low byte first). |
| `$99` | The BASIC token for `PRINT`. BASIC stores keywords as single-byte tokens. |
| `$22, $93, $22` | Quote, clear screen character ($93), quote — the string `"{CLR}"`. |
| `$3a` | Colon — separates BASIC statements on one line. |
| `$9e` | The BASIC token for `SYS`. |
| `"2066"` | The address argument, stored as ASCII text (not a binary number). |
| `$00` | Zero byte marks end of this BASIC line. |
| `$00, $00` | Null pointer (no next line) marks end of program. |

This creates the equivalent of typing: `10 PRINT "{CLR}":SYS 2066`

When BASIC runs this, `PRINT "{CLR}"` clears the screen, then `SYS 2066` transfers execution to address 2066 ($0812), where our machine code begins. The `SYS` command also stores the return location on the stack to be accessed later.
...
|---------|-------------|--------------|
| $0812 | `lda #$00` | Put the value 0 (black) into the accumulator |
| $0814 | `sta $d020` | Write the accumulator to the border color register |
| $0817 | `sta $d021` | Write the accumulator to the background color register (A still holds 0) |
| $081A | `rts` | Return to BASIC |
...
The compiled `hello.prg` is 28 bytes. Here's the hexdump:

```
00000000: 0108 1008 0a00 9922 9322 3a9e 3230 3636  .......".":.2066
00000010: 0000 00a9 008d 20d0 8d21 d060            ...... ..!.`
```
...
| 0002 | `10 08` | BASIC: next line pointer ($0810) |
| 0004 | `0a 00` | BASIC: line number 10 |
| 0006 | `99` | BASIC: PRINT token |
| 0007 | `22 93 22` | Quote, clear screen ($93), quote |
| 000a | `3a` | BASIC: colon |
| 000b | `9e` | BASIC: SYS token |
| 000c | `32 30 36 36` | ASCII "2066" |
| 0010 | `00` | BASIC: end of line |
| 0011 | `00 00` | BASIC: end of program |
| 0013 | `a9 00` | LDA #$00 |
| 0015 | `8d 20 d0` | STA $d020 |
| 0018 | `8d 21 d0` | STA $d021 |
| 001b | `60` | RTS |
